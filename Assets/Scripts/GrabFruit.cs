            ...
            if (hit.distance <= range)
            {
                ...
                if (Input.GetMouseButton(0))
                {
                    ...
                    if (currentHoldTime >= grabtime)
                    {
                        ...
                        Debug.Log(grabbedFruit.name);
                        grabbedFruit.transform.localScale = grabbedFruit.transform.localScale * 0.3f; // 크기 줄이기
                        grabbedFruit.SetActive(true); // 복제된 과일 활성화

                        // 복제된 과일은 카트 위에서 랜덤으로 떨어지기
                        Vector3 randomOffset = shoppingCart.transform.right * Random.Range(-0.4f, 0.4f) + shoppingCart.transform.forward * Random.Range(-0.2f, 0.2f);
